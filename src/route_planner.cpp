...
bool RoutePlanner::Compare(const RouteModel::Node *a, const RouteModel::Node *b) {
    ...
    auto f_value2 = b->g_value + b->h_value;
...
// Tips:
// - This method should take theRoutePlanner current (final) node as an argument and iteratively follow the
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    path_found.emplace_back(*iterate_node);
    do{
        distance+=iterate_node->distance(*iterate_node->parent);
        iterate_node=iterate_node->parent;
        path_found.emplace_back(*iterate_node);
    }while(iterate_node!=start_node);
